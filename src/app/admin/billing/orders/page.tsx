...
export default function AdminOrdersPage() {
  ...
  return (
    <div className="space-y-8 text-white">
      <div>
        <p className="text-sm uppercase tracking-[0.3em] text-[#D9B86C]">Order Management</p>
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.id} className="border-b border-white/10 text-white/75">
                <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-[#D9B86C] text-[#15100e]"><CreditCard className="size-4" /></div><div><p>{order.user.name || order.user.email}</p><p className="text-xs text-white/45">{order.user.email}</p></div></div></td>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {transactions.map((tx) => (
            <div key={tx.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-2 text-xs text-[#D9B86C]">{tx.status} · signature {tx.signatureVerified ? "ok" : "invalid"}</p>
            </div>
          ))}
        </div>
      </div>
